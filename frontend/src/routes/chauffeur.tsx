...
import { Bus, ShieldCheck, User, LogOut } from "lucide-react";
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <div className="flex w-full flex-col md:hidden">
        <header className="sticky top-0 z-20 flex h-14 items-center justify-between border-b bg-card px-4">
          <h1 className="text-lg font-bold text-bus">FianaCity Chauffeur</h1>
        ...
        <nav className="flex border-b bg-card">
          {items.map((it) => {
            const active = path.startsWith(it.to);
            return (
              <Link key={it.to} to={it.to} className={`flex-1 px-3 py-2.5 text-center text-sm ${active ? "border-b-2 border-bus font-semibold text-bus" : "text-muted-foreground"}`}>
                {it.label}
              </Link>
            );
          })}
